...
const supabase = (dbReady && window.supabase && typeof window.supabase.createClient === "function")
...
async function loadPublicData(){
  if(!supabase){ people=[]; return; }
  const {data:p,error}=await supabase.from("people").select("*").order("created_at",{ascending:true});
  if(error){console.error(error);return;}
  people=p||[];
  const {data:m,error:me}=await supabase.from("memories").select("*").order("sort_order",{ascending:true}).order("created_at",{ascending:true});
  if(me){console.error(me);return;}
  memoriesByPerson={};
  (m||[]).forEach(x=>{
    (memoriesByPerson[x.person_id] ||= []).push({...x,src:supabase.storage.from(cfg.photoBucket).getPublicUrl(x.storage_path).data.publicUrl});
  });
}
...
async function openAdmin(){document.getElementById("adminModal").classList.remove("hidden");if(!supabase){showAdminSetup();return;}const {data:{session}}=await supabase.auth.getSession();adminSession=session;if(!session){showLogin();return;}await refreshAdmin();showDashboard();}
...
function bindPersonForm(id){document.getElementById("personForm").onsubmit=async e=>{e.preventDefault();const slug=(id?adminPeople.find(x=>x.id===id).slug:fName.value.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"person-"+Date.now());const payload={slug,name:fName.value.trim(),relationship:fRel.value.trim(),intro:fIntro.value,letter:fLetter.value,reward_title:fRewardTitle.value,reward_text:fRewardText.value,score:Number(fScore.value),status:fStatus.value,memories:Number(fMemories.value),updated_at:new Date().toISOString()};let result=id?await supabase.from("people").update(payload).eq("id",id):await supabase.from("people").insert(payload);if(result.error){alert(result.error.message);return;}await loadPublicData();await refreshAdmin();showDashboard();};}
async function deletePerson(id){const p=adminPeople.find(x=>x.id===id);if(!confirm(`Delete ${p.name} and all their memory records?`))return;const {error}=await supabase.from("people").delete().eq("id",id);if(error){alert(error.message);return;}await loadPublicData();await refreshAdmin();showDashboard();}
async function photoManager(id){const p=adminPeople.find(x=>x.id===id);const {data:ms}=await supabase.from("memories").select("*").eq("person_id",id).order("sort_order").order("created_at");const list=ms||[];document.getElementById("adminContent").innerHTML=`<div class="eyebrow">MEMORY VAULT</div><h2>${esc(p.name)} 📸</h2><p class="muted">Upload a photo, then add its caption. Images are stored in your Supabase storage bucket.</p><form id="uploadForm" class="admin-form"><label>Photo<input id="photoFile" type="file" accept="image/*" required></label><label>Caption<input id="photoCaption" value="A memory worth keeping. ❤️"></label><button class="btn gold" type="submit">UPLOAD PHOTO</button></form><div class="photo-admin-grid">${list.map(m=>{const src=supabase.storage.from(cfg.photoBucket).getPublicUrl(m.storage_path).data.publicUrl;return `<div class="photo-admin"><img src="${esc(src)}"><input id="cap-${m.id}" value="${esc(m.caption)}"><div class="navrow compact"><button class="btn small" onclick="updateCaption('${m.id}')">Save caption</button><button class="btn small ghost" onclick="deletePhoto('${m.id}','${esc(m.storage_path)}','${id}')">Delete</button></div></div>`;}).join("")||`<div class="admin-item">No photos yet.</div>`}</div><div class="navrow"><button class="btn ghost" onclick="showDashboard()">← Dashboard</button></div>`;document.getElementById("uploadForm").onsubmit=async e=>{e.preventDefault();const file=photoFile.files[0];if(!file)return;const ext=(file.name.split(".").pop()||"jpg").toLowerCase().replace(/[^a-z0-9]/g,"");const path=`${p.slug}/${crypto.randomUUID()}.${ext}`;const up=await supabase.storage.from(cfg.photoBucket).upload(path,file,{contentType:file.type||"image/jpeg",upsert:false});if(up.error){alert(up.error.message);return;}const {data:existing}=await supabase.from("memories").select("sort_order").eq("person_id",id).order("sort_order",{ascending:false}).limit(1);const next=(existing?.[0]?.sort_order||0)+1;const ins=await supabase.from("memories").insert({person_id:id,storage_path:path,caption:photoCaption.value,sort_order:next});if(ins.error){await supabase.storage.from(cfg.photoBucket).remove([path]);alert(ins.error.message);return;}await loadPublicData();photoManager(id);};}
async function updateCaption(id){const val=document.getElementById(`cap-${id}`).value;const {error}=await supabase.from("memories").update({caption:val}).eq("id",id);if(error)alert(error.message);else{await loadPublicData();alert("Caption saved ❤️");}}
async function deletePhoto(memoryId,path,personId){if(!confirm("Delete this photo?"))return;const a=await supabase.from("memories").delete().eq("id",memoryId);if(a.error){alert(a.error.message);return;}const b=await supabase.storage.from(cfg.photoBucket).remove([path]);if(b.error)console.warn(b.error);await loadPublicData();photoManager(personId);}
...
(async()=>{
  try {
    if(supabase){ await loadPublicData(); }
    render();
  } catch (error) {
