...
function rewardImageSrc(p){return p.reward_image_path && sb ? sb.storage.from(cfg.photoBucket).getPublicUrl(p.reward_image_path).data.publicUrl : null;}
...
function bindPersonForm(id){document.getElementById("personForm").onsubmit=async e=>{
  ...
  if(file){
    ...
    const up=await sb.storage.from(cfg.photoBucket).upload(path,file,{contentType:file.type||"image/jpeg",upsert:false});
  ...
  let result=id?await sb.from("people").update(payload).eq("id",id):await sb.from("people").insert(payload);
  if(result.error){alert(result.error.message);return;}
  if(newPath!==oldPath && oldPath){await sb.storage.from(cfg.photoBucket).remove([oldPath]);}
